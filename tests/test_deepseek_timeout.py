...
def test_deepseek_timeout(api_key, api_url=None, model_id=None, timeout_seconds=60,
    ...
    while retry_count < max_retries:
        ...
        try:
            logger.info(
                f"测试 #{retry_count + 1}: 开始调用 Deepseek API (超时设置: {timeout_seconds}秒)")
            ...
            attempt_time = time.time() - attempt_start
            logger.warning(
                f"测试 #{retry_count + 1}: 请求超时! 耗时: {attempt_time:.2f}秒")
            ...
            attempt_time = time.time() - attempt_start
            logger.error(
                f"测试 #{retry_count + 1}: 连接错误! 耗时: {attempt_time:.2f}秒, 错误: {str(e)}")
            ...
            attempt_time = time.time() - attempt_start
            logger.error(
                f"测试 #{retry_count + 1}: 请求异常! 耗时: {attempt_time:.2f}秒, 错误: {str(e)}")
            ...
            attempt_time = time.time() - attempt_start
            logger.error(
                f"测试 #{retry_count + 1}: 未知错误! 耗时: {attempt_time:.2f}秒, 错误: {str(e)}")
    ...
    if result["success"]:
        logger.info(
            f"测试成功完成! 总耗时: {result['total_time']:.2f}秒, 尝试次数: {result['attempts']}")
    else:
        logger.warning(
            f"测试失败! 总耗时: {result['total_time']:.2f}秒, 尝试次数: {result['attempts']}, 错误: {result['error']}")
...
def save_test_result(result, output_dir="data/tests"):
    ...
    try:
        ...
        filename = os.path.join(
            output_dir, f"deepseek_test_{status}_{timestamp}.json")
